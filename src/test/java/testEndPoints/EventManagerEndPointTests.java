...
public class EventManagerEndPointTests extends BaseTest {
	...
	@Test(priority = 1, description = "Retrieve all events on the system")
	public void testGetRequest() throws IOException {
		...
		response.then().statusCode(200).body("status", equalTo("SUCCESS")).body("data", notNullValue())
	...
	@Test(priority = 2, description = "Creating a new booking")
	public void createNewBookingTest() throws IOException {
		...
		response.then().statusCode(200).body("reference", notNullValue()).body("status", equalTo("SUCCESSFUL"))
				.body("eventName", notNullValue()).body("fee", greaterThan(0f));
	...
	@Test(priority = 3, description = "Cancel a Booking")
	...
	@Test(priority = 4, description = "Passing a parameter from runtime")
	public void parameterizationTestCreateNewBooking(String bookingType) throws IOException {
		String endpoint = Routes.post_booking_url;
		String eventReferenceId = readFromFile(ConfigReader.get("EVENT_ID_PATH"));
	...
	@Test(dataProvider = "bookingTypes", dataProviderClass = BookingDataProvider.class, priority = 5, description = "Passing different parameters on the same test")
	...
	@Test(priority = 6, description = "Verify available capacity reduces after booking")
	public void testAvailableCapacityReducesAfterBooking() throws IOException {
		...
		Assert.assertEquals(availableAfter, availableBefore - 1, "Available capacity did not reduce after booking");
